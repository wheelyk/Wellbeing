...
- [ ] Establish base Tailwind design tokens (colors, spacing, font sizes) for a calm, high-contrast, low-clutter UI, and reusable primitives: `Button`, `Card`, `RatingScale`, `Modal`, `TextField`, `DatePicker`. (Design tokens live in `frontend/src/index.css`; `Button`/`Card`/`TextField` already existed, `Modal` is now built too — a real focus-trapping dialog, added for the Dashboard's dialog-based Quick Add. `RatingScale` and `DatePicker` still don't exist as their own components — each form builds its own inline rating control and uses a native `datetime-local` input directly. See [IMPLEMENTATION_LOG.md](IMPLEMENTATION_LOG.md).)
