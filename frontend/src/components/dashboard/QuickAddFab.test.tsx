...
describe("QuickAddFab", () => {
  ...
  it("shows all three built-in items plus More… when no props are given (every existing call site)", async () => {
    const user = userEvent.setup();
    render(<QuickAddFab />);
  ...
  it("omits a disabled built-in category's item, but always keeps More…", async () => {
    const user = userEvent.setup();
    render(<QuickAddFab moodEnabled={false} symptomEnabled={false} medicationEnabled />);
