...
describe("MedicationSection", () => {
  ...
  it("shows Load less once more than a page is loaded, and it collapses back without a new fetch", async () => {
    ...
    render(<MedicationSection />);
    await screen.findAllByText(/ibuprofen — taken/i);
    ...
    expect(screen.queryByText(/ibuprofen — not taken/i)).not.toBeInTheDocument();
    expect(screen.getAllByText(/ibuprofen — taken/i)).toHaveLength(10);
    expect(screen.getByRole("button", { name: /load more/i })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /load less/i })).not.toBeInTheDocument();
  });
