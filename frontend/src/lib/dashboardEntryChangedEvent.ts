// A tiny, deliberately loose contract between the three Dashboard sections (MoodSection,
// SymptomSection, MedicationSection) plus CategorySection, and DashboardSummary - dispatched as a
...
// doesn't need to know these sections exist, and a section doesn't need to know
...
export type DashboardEntryChangedType = "mood" | "symptom" | "medication" | "category";
...
// numbers (mood/symptom/medication counts, streak, recent entries) can each change from any one
// of these types changing, so every section's event is relevant here, not just one.
